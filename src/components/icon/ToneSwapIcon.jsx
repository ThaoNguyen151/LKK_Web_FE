...
/**
 * Icon P/W chồng nhau — đổi khi parent có `group/icon` + hover.
 * @param {object} props
  ...
  return (
    <span className={cn('relative inline-block h-4 w-4 shrink-0', className)}>
      <IconImg
        src={baseSrc}
        className={cn(
          'absolute inset-0 m-auto h-full w-full object-contain transition-opacity duration-300 group-hover/icon:opacity-0',
          imgClassName
        )}
      />
      <IconImg
        src={hoverSrc}
        className={cn(
          'absolute inset-0 m-auto h-full w-full object-contain opacity-0 transition-opacity duration-300 group-hover/icon:opacity-100',
          imgClassName
        )}
      />
    </span>
...
export const ICON_TOOL_BUTTON_VARIANTS = {
  activities:
    'group/icon flex h-8 w-8 items-center justify-center rounded-full bg-transparent transition-colors duration-300 hover:bg-brand-home1',
  news: 'group/icon flex h-9 w-9 items-center justify-center rounded-full bg-transparent transition-colors duration-300 hover:bg-white',
}
