 ...
 * @param {number} [page]
 */
export function newsPagePath(page = 1) {
  if (page <= 1) return '/news'
  return `/news/${page}`
}

/**
 * Parse số trang từ hash path `/news` hoặc `/news/2`.
 * @param {string} route
 */
export function parseNewsPage(route) {
  if (!route.startsWith('/news')) return 1
  const rest = route.slice('/news'.length).replace(/^\//, '')
  if (!rest) return 1
...
/** Sample: đủ nhiều bài (id unique) để phân trang 20/trang */
const SAMPLE_TOTAL = 45

/** @type {NewsItem[]} */
export const NEWS_ITEMS = Array.from({ length: SAMPLE_TOTAL }, (_, index) => {
  const seed = NEWS_SEED[index % NEWS_SEED.length]
  const batch = Math.floor(index / NEWS_SEED.length)
  return {
    ...seed,
    id: String(index + 1),
    title: batch === 0 ? seed.title : `${seed.title} (${batch + 1})`,
  }
})
